                                       ...
                                       std::string file_name)
{
    ...
    uint32_t size = k4a_image_get_size(transformed_color_image);
    printf("color_to_depth %d, %d\n", size, k4a_image_get_format(transformed_color_image));
    pFile = fopen ("/mnt/mrob_tmpfs/_color_kek2.bin", "w");
    ...
    pFile = fopen ("/mnt/mrob_tmpfs/_depth_kek2.bin", "w");
                                       ...
                                       std::string file_name)
{
    ...
    printf("depth_to_color %d, %d\n", size, k4a_image_get_format(color_image));
    pFile = fopen ("/mnt/mrob_tmpfs/_color_kek.jpg", "w");
    ...
    pFile = fopen ("/mnt/mrob_tmpfs/_depth_kek.bin", "w");
...
static int playback(char *input_path, int timestamp = 1000, std::string output_filename = "output.ply")
{
    ...
    // Compute color point cloud by warping depth image into color camera geometry
    if (point_cloud_depth_to_color(transformation, depth_image, color_image, output_filename) == false)
    {
    ...
    //////////////////
    // Compute color point cloud by warping depth image into color camera geometry
    if (point_cloud_color_to_depth(transformation, depth_image, uncompressed_color_image, output_filename) == false)
    {
        printf("Failed to transform depth to color\n");
        goto Exit;
    }
